# -*- coding: utf-8 -*-
"""
Created on Wed Nov 29 08:26:08 2023
...
# snow level https://forecast.weather.gov/MapClick.php?zoneid=CAZ069#:~:text=Snow%20level%205500%20feet.,winds%20up%20to%2010%20mph.
...
CSS_df = pd.read_csv('C:\\Users\\jburdick\\Box\\01. jacob.burdick Workspace\Documents\\TNF dashboard\\Palisades Tahoe.csv')
CSS_df['datetime'] = pd.date_range(start = '10/1/2019', end = '9/30/2020', freq = 'D')
CSS_df = CSS_df.drop([151])
CSS_df = CSS_df.reset_index()

y= signal.savgol_filter(CSS_df['10%'], # smoothes data because day of year repeated over multiple times a day
                        53, # window size used for filtering
...
a[a < 0] = 0
CSS_df['10%'] = a
CSS_df.to_csv('C:\\Users\\jburdick\\Box\\01. jacob.burdick Workspace\Documents\\TNF dashboard\\PT_SWE.csv')

y= signal.savgol_filter(CSS_df['30%'], # smoothes data because day of year repeated over multiple times a day
                        53, # window size used for filtering
                        2), # order of fitted polynomial ,
l = list(y)
a = l.pop(0)
...
y= signal.savgol_filter(CSS_df['70%'], # smoothes data because day of year repeated over multiple times a day
                        53, # window size used for filtering
                        2), # order of fitted polynomial ,
l = list(y)
a = l.pop(0)
...
y= signal.savgol_filter(CSS_df['90%'], # smoothes data because day of year repeated over multiple times a day
                        53, # window size used for filtering
                        2), # order of fitted polynomial ,
l = list(y)
a = l.pop(0)
...
fig.add_trace(go.Scatter(
    x=JBR_365['datetime'], y= JBR_365['Max'],
    name = '90th percentile-max',
    visible = 'legendonly',
    hoverinfo = 'skip',
    mode='lines',
...
#wtelev line
fig.add_trace(go.Scatter(
    x=JBR_365['datetime'], y= JBR_365['2024'],
    name = 'SWE',
    mode='lines',
    line=dict(width=2, color='black'),
))
...
y_min = -1
y_max = JBR_365['2024'].max() + 2
for i in JBR_365.index:
    if np.isnan(JBR_365.loc[i,'2024']) == False:
        x_max = i + 10
x_min = JBR_365.loc[0,'datetime']
if x_max > 355:
    x_max = 364
x_max = JBR_365.loc[x_max,'datetime']
